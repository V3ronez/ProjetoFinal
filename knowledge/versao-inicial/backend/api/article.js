...
module.exports = app => {
    ...
    const get = async (req, res) => {
        ...
        const count = parseInt(result.count) //transforma string em inteiro

        app.db('articles')
            .select('id', 'name', 'description')
            .limit(limit).offset(page * limit - limit)
            .then(articles => res.json({ data: articles, count, limit })) //facilita a renderizar o paginador
            .catch(err => res.status(500).send(err))
    }
    const getById = (req, res) => {
        app.db('articles')
            ...
            .first()
            .then(article => {
                article.content = article.content.toString() //converte para string o artigo que vem em binario
    ...
    const getByCategory = async (req, res) => {
        ...
        app.db({ a: 'articles', u: 'users' }) // a: apelido para article e u: para user
